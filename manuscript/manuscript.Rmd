...
As the models contained only categorical variables but no meaningful reference category,
we estimated a model _without_ an intercept,
and instead included an equal number of dummies to the number of unique categories (ANOVA specification instead of regression specification).
This way, the effect of each dummy reflects the mean value for that category.
To further aid interpretability, the dependent variable (effect size) was centered around the random-effects meta-analysis estimated overall effect.
This changes the interpretation of the effect of each dummy, such that it reflects the deviation of that category from the overall effect size as estimated in the random-effects meta-analysis.
...
modres <- lapply(processes, function(p){
  df <- dat[dat$Process == p, ]
  modmat <- model.matrix(~-1+., df[mods])[,-1]
  ...
  res <- rma(yi = yi_centered,
             vi = df$vi,
             mods = modmat, intercept = F)
  vifs <- vif(res)
  ...
  tabres$VIF <- c(vifs$vif, NA)
  write.csv(tabres, paste0("rma_mods_", p, ".csv"))
  
  
  moddat <- data.frame(yi = yi_centered, vi = df$vi, modmat)
  moddat$study <- 1
