...
run_everything = TRUE
sensitivity = FALSE
sens_papers <- "{{sens_study}}" #c("Camp 2012 (1)", "Camp 2012 (2)", "Camp 2012 (3)", "Camp 2012 (4)", "Fitzgerald 2015", "Gottschalk 2018", "Ju 2017", "Xiong 2015 (1)", "Xiong 2015 (2)")
library("worcs")
library(ggplot2)
library(kableExtra)
library(DT)
...
load_data()
if(sensitivity){
  dat <- dat[!dat$article.code %in% sens_papers, ]
}
...
dat <- escalc(measure = "SMD",
         n2i = dat$number.of.animals.in.control.group,
         n1i = dat$number.of.animals.in.intervention.group,
         m2i = dat$mean.control.group,
         m1i = dat$mean.experimental.group,
         sd2i = dat$sd.control.group,
         sd1i = dat$sd.experimental.group,
         data = dat)
...
```{r, results = "asis"}
cat("**This is a sensitivity analysis; the study {{sens_study}} has been removed from the data.**")
```
...
tb <- lapply(1:length(processes), function(i){
  if(is.null(mlm[[i]])) return(NULL)
  tb <- mlm[[i]]$res
  tb <- rbind(tb, data.frame(Variance = "I2",
                             estimate.intrcpt = mlm[[i]]$mod$I2,
                             ci.lb = NA,
                             ci.ub = NA,
                             V4 = mlm[[i]]$mod$QEp))
...
tb <- do.call(rbind, tb)
tb$CI <- NA
tb$CI[!is.na(tb$ci.lb)] <- tidySEM::conf_int(lb = tb$ci.lb[!is.na(tb$ci.lb)], ub = tb$ci.ub[!is.na(tb$ci.lb)], digits = 2)
...
The analyses below used the following reference categories:

```{r}
refcat <- structure(list(Variable = c("ssri", "freq", "disease", "species", "test"
), Reference = c("FLX", "chronic", "stress", "rat", "CF")), class = "data.frame", row.names = c(NA, -5L))

datatable(
  refcat,
  rownames = FALSE,
  caption = "Reference category for each variable")
```
...
modres <- lapply(processes, function(p){
  ...
  df <- dat[dat$Process == p, ]
  if(nrow(df) == 0) return(NULL)
  df[refcat$Variable] <- lapply(refcat$Variable, function(n){
    v <- df[[n]]
    v <- relevel(v, ref = refcat$Reference[refcat$Variable == n])
    droplevels(v)
  })
  # refcat <- data.frame(do.call(rbind,lapply(names(df[catvars]), function(n){c(n, levels(df[[n]])[1])})))
  # names(refcat) <- c("Variable", "Reference")
  df_mods <- df[mods]
  constcols <- sapply(df_mods, function(i){length(unique(i))}) == 1
  df_mods <- df_mods[, !constcols, drop = FALSE]
  modmat <- model.matrix(~., df_mods)
  ...
  res <- rma(yi = yi_notcentered,
             ...
             mods = modmat)
  vifs <- vif(res)
  saveRDS(res, paste0("rma_", p, ".RData"))
  ...
  pma <- try({brma(yi ~., study = "study", data = moddat, standardize = standardize)})
  if(inherits(pma, "try-error")){
    pma <- NULL
    sumpma <- data.frame(NA, NA)
  } else {
...
```{r, eval = run_everything}
refcat <- structure(list(Variable = c("ssri", "freq", "disease", "species", "test"
), Reference = c("FLX", "chronic", "stress", "rat", "CF")), class = "data.frame", row.names = c(NA, 
-5L))
library(ggridges)
for(p in processes){
  ...
  vars <- c("ssri", "freq", "disease", "species", "test")
  vars <- vars[sapply(vars, function(v){any(grepl(v, colnames(res$X)))})]
  for(v in vars){
    catdf <- res$X[, startsWith(colnames(res$X), v), drop = F]
    splt <- apply(catdf, 1, which.max) + 1
    splt[rowSums(catdf == 0) == ncol(catdf)] <- 1
    labs <- c(refcat$Reference[refcat$Variable == v],
...
plotnames <- paste0(
  "group_plot_", 
  rep(vars, length(processes)),
  "_",
  rep(processes, each = length(vars)),
  ".png")
